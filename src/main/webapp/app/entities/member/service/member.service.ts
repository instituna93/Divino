...
type RestOf<T extends IMember | NewMember> = Omit<T, 'createdOn' | 'updatedOn' | 'deletedOn' | 'birthday'> & {
...
export class MemberService {
  ...
  protected convertDateFromClient<T extends IMember | NewMember | PartialUpdateMember>(member: T): RestOf<T> {
    return {
      ...
      updatedOn: member.updatedOn?.toJSON() ?? null,
      deletedOn: member.deletedOn?.toJSON() ?? null,
      birthday: member.birthday?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restMember: RestMember): IMember {
    return {
      ...
      updatedOn: restMember.updatedOn ? dayjs(restMember.updatedOn) : undefined,
      deletedOn: restMember.deletedOn ? dayjs(restMember.deletedOn) : undefined,
      birthday: restMember.birthday ? dayjs(restMember.birthday) : undefined,
    };
  }
