...
export class InstrumentComponent implements OnInit {
  ...
  trackId = (_index: number, item: IInstrument): number => this.instrumentService.getInstrumentIdentifier(item);
  ...
  protected fillComponentAttributesFromResponseBody(data: IInstrument[] | null): IInstrument[] {
    ...
    if ('prev' in this.links) {
      const instrumentsNew = this.instruments ?? [];
      if (data) {
        for (const d of data) {
          if (instrumentsNew.map(op => op.id).indexOf(d.id) === -1) {
            instrumentsNew.push(d);
          }
        }
      }
